...
namespace RemoteData
{
	...
	public class Activity1 : Activity
	{
		...
		protected override void OnCreate (Bundle bundle)
		{
			...
			/* ???
			 * --> caused by the windows >< mac conversion (strange that this just started to occur)
			 */
		...
		private void ChangeTweetCount (object sender, EventArgs e)
		{
			 ...
			 * it's a good practice to always include a default statement, even if it just 'breaks'
			 */
			switch (view.Id) {
			case Resource.Id.btnL:
				if (tweetCount > 5) {
					 ...
					 * 
					 * of course the code is ok. shorter = better is not an absolute truth :)
					 * readability of the code is also important. In this case it's a little nitpicking but the point is important. Reducing code, should still keep it readable, after 6 months or when someone
					 * else reads it, the general idea should be clear.
					 * (again, in this case, not so much of a problem, but hopefully you cathc my drift :))
					 */
					tvTweetCount.Text = String.Format ("{0}",--tweetCount);
				}
				break;
